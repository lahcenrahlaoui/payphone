...
public class RingProgressIndicatorSkin implements Skin<RingProgressIndicator> {
    ...
    // hello hadi hiya Time li 9otlk 3liha
   int Time = Client.circleTime;
    public RingProgressIndicatorSkin(final RingProgressIndicator indicator) {
        ...
        this.indicator.progressProperty().addListener((o, oldVal, newVal) -> {
            if (newVal.intValue() >= 0) {
                ...
                fillerArc.setLength(newVal.intValue() * -360/Time);// hello hna rani mbedel bla bla bla
            }
        });
    ...
    private void initTransition() {
        ...
        transition.setByAngle(Time);
        System.out.println("****************************"+ Time + "***********fasfa******* ");
    }

    private void initFillerArc() {
        fillerArc.setManaged(false);   //// hello  hadi kon dirha true yetem7alek l project .. rod balek  treja3ha true
        fillerArc.getStyleClass().add("ringindicator-filler");
        fillerArc.setStartAngle(90);
        fillerArc.setLength(indicator.getProgress() * -360/Time);// hello rani kateb Time f plasset 100
    }
